...
/** 数据 */
export type XSelectData<TValue> = Array<
...
// data、service 必填一个
export type XSelectProps<TValue> = RequireExactlyOne<
  Merge<
    // mode: 类型不友好，新增组件代替
    ...
    StrictOmit<
      SelectProps<TValue>,
      'mode' | 'labelInValue' | 'optionLabelProp'
    >,
    {
      /** 选项数据 */
...
export type XSelectMultipleProps<TValue> = Merge<
  XSelectProps<TValue>,
  {
    /** 初始选中值 */
    defaultValue?: TValue[]
...
export type XSelectTagsProps<TValue> = Merge<
  XSelectProps<TValue>,
  {
    /** 初始选中值 */
    defaultValue?: TValue[]
